...
class UpdateGameWithMoveHandlerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->updateGameWithMove = new UpdateGameWithMove($this->gameRepository);
    }

    public function testShouldUpdateGameWithNewMove()
    {
        $updateGameWithMoveHandler = new UpdateGameWithMoveHandler($this->updateGameWithMove);
        $updateGameWithMoveHandler->__invoke(new UpdateGameWithMoveCommand(self::FAKE_GAME_UUID, 0, 0));
        $updatedGame = $this->gameRepository->findById($this->gameId);
        $this->assertEquals(
            $updatedGame->board(),
            new SquareBoard(
                new CellCollection(
